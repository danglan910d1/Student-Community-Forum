...
export const getApprovedTopics = async (req: Request, res: Response) => {
  try {
    ...
    const topics = await Topic.find({ status: "approved" }) // 3. Chỉ chọn các trường cần thiết cho public
      .select("name slug description"); // 4. Phản hồi thành công
...
export const createTopic = async (
  ...
  try {
    ...
    const adminId = req.userId; // ID của Admin đã được gán // 2. Kiểm tra tính hợp lệ cơ bản
    ...
    const slug = slugify(name, { lower: true, locale: "vi" }); // 5. Tạo Topic trong Database
...
export const getAllTopicsForAdmin = async (
  ...
  try {
    ...
    const topics = await Topic.find() // 2. Sắp xếp theo thời gian tạo mới nhất
...
export const updateTopic = async (
  ...
  try {
    ...
    const updateFields: any = {}; // 2. Xử lý trường name và tự động cập nhật slug
    ...
    if (description !== undefined) updateFields.description = description; // 4. Xử lý trường status (Duyệt/Từ chối)
    if (status) updateFields.status = status; // 5. Kiểm tra nếu không có trường nào được cung cấp
